...
url = f"{api_url}/api/v1/data-tables/{table_id}/rows/{row_id}"
req = urllib.request.Request(url, headers=headers, method="DELETE")
try:
    with urllib.request.urlopen(req) as resp:
        print("DELETE row 228 SUCCESS ->", resp.read().decode())
except urllib.error.HTTPError as e:
    print(f"DELETE row 228 FAILED -> {e.code} {e.reason}: {e.read().decode()}")
