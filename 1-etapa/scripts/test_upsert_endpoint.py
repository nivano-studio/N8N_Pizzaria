...
upsert_payloads = [
    {"filter": {"name": "CALABRESA - Brotinha"}, "data": {"name": "CALABRESA - Brotinha", "price": "15"}},
    {"filter": {"name": "CALABRESA - Brotinha"}, "data": [{"name": "CALABRESA - Brotinha", "price": "15"}]},
    {"filter": "name", "data": [{"name": "CALABRESA - Brotinha", "price": "15"}]},
    {"filter": ["name"], "data": [{"name": "CALABRESA - Brotinha", "price": "15"}]},
]
...
for p in upsert_payloads:
    ...
    try:
        with urllib.request.urlopen(req) as resp:
            print(f"SUCCESS upsert with {p['filter']} -> {resp.read().decode()}")
    except urllib.error.HTTPError as e:
        print(f"FAILED upsert with {p['filter']} -> {e.code} {e.reason}: {e.read().decode()[:150]}")
